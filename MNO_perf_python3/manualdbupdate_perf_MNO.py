...
# Variant: Perf-MNO  carrier extracted from TestCaseNumber (not TestcaseVersion)
# @Author : Abhi Goyal
##===========================================================================##
...
def get_final_verdict_reason(soup):
	for p in soup.find_all('p', class_='result'):
		if 'Final Verdict' in p.get_text():
			...
			reason_match = re.search(r'\((.+)\)', full_text) or re.search(r'\((.+)\)', full_text, re.DOTALL)
			if reason_match:
...
def get_carrier_from_testcase_number(testcase_number):
	# TestCaseNumber format: "MNO-Band_XX-YYY.ZZZ" - carrier is the prefix before the first "-"
	return testcase_number.split("-")[0].strip()

def get_carrier_from_group_description(group_description):
	upper = group_description.upper()
	for op in ['AT&T', 'TMO', 'ATT', 'VZW']:
		if op.upper() in upper:
			return op
	return None
	...
	if os.path.exists(json_file_path) :
		...
		if ((cdate <= ydate and cdate >= enddate)):
			...
			try:
				...
				# Carrier: prefer GroupDescription if it contains a known operator name, else fall back to TestCaseNumber prefix
				group_desc = str(report_json['header'].get('Application', {}).get('GroupDescription', '')).strip()
				Carrier = get_carrier_from_group_description(group_desc) or get_carrier_from_testcase_number(testcaseid)
				logging.info("Carrier from GroupDescription=%s, Carrier=%s" % (group_desc, Carrier))
				# start and end time
				...
				# TE BUILD ID - take only the semver part before the "+" (drops the git-SHA suffix)
				kaf_text=kaf_version_from_constest_db_set(root)
				te_version = TestcaseVersion.split("+")[0].strip()
				if kaf_text:
					TE_Build = kaf_text+","+te_version
				else:
					TE_Build=te_version
				if TE_Build:
				...
				if result != 'PASS' and os.path.exists(htm_file_path):
					try:
						reason = get_final_verdict_reason(BeautifulSoup(open(htm_file_path, encoding='utf-8', errors='ignore'), "html.parser"))
					except:
...
def main():
	...
	ROOT_DIRS = [
		r"\\10.142.147.96\ContestReports"]
...
if __name__=="__main__":
	try:
		...
		filename="RAS_contest_perf_MNO_update_log"+taskstarttime.replace(":",'_')+".txt"
		...
		ydate=""
		enddate=""
		taskname="R&S Contest Perf MNO Update Script"
