...
def getexecmode(Texthtml):
	index=str(Texthtml).index("Automation Configuration File:")
	index=str(Texthtml).index(":",index)
	index0=str(Texthtml).index("\n",index)
	Modestring=str(Texthtml)[index+1:index0]
...
def gettestcaseid(Texthtml):
	...
	index1=str(Texthtml).index(":",index1)
	index2=str(Texthtml).index("\n",index1)
	testcaseid=str(Texthtml)[index1+1:index2]
...
def getcarrier(Texthtml):
	index3=str(Texthtml).index("File:")
	index3=str(Texthtml).index(":",index3)
	index4=str(Texthtml).index("\n",index3)
	sourcepath= str(Texthtml)[index3+1:index4]
	#print sourcepath
	Carrier=""
	try:
...
def date_formater_anite(sdate):
	...
	if m1:
		print("match1")
		d1 = datetime.datetime.strptime(str(sdate), "%m/%d/%Y %I:%M:%S %p")
		...
		if m2:
			print("match2")
			d1 = datetime.datetime.strptime(str(sdate), "%d-%b-%y %I:%M:%S %p")
			sdate=str(datetime.datetime.strftime(d1,"%d-%m-%Y:%H:%M:%S"))
		else:
			...
			if m3:
				print("match3")
				d1 = datetime.datetime.strptime(str(sdate), "%d/%m/%Y %H:%M:%S")
				sdate = str(datetime.datetime.strftime(d1, "%d-%m-%Y:%H:%M:%S"))
	return sdate
...
def getstarttime(Texthtml):
	index5=str(Texthtml).index("Test Case Started at:")
	index5=str(Texthtml).index(":",index5)
	index6=str(Texthtml).index("\n",index5)
	starttime=str(Texthtml)[index5+1:index6]
...
def getendtime(Texthtml,file_path,starttime):
	try:
		index7=str(Texthtml).index("TestCase Completed at:")
		index7=str(Texthtml).index(":",index7)
		index8=str(Texthtml).index("\n",index7)
		endtime=str(Texthtml)[index7+1:index8]
...
def getresult(Texthtml):
	index9=str(Texthtml).rindex("Final Verdict:")
	index9=str(Texthtml).index(":",index9)
	index10=str(Texthtml).index("\n",index9)
	result=str(Texthtml)[index9+1:index10]
	result=result.strip()
	result=result.strip('\r\n')
	return result
...
def getendtimefromrtt(file_path1,starttime):
	with open(file_path1, 'r', errors='ignore') as rtfile:
